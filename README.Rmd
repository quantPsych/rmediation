...
1.  You have already performed a mediation analysis using a structural equation model (SEM) software and you have coefficient and indirect effect estimates along with their standard errors and covariance matrix of the coefficients.
...
2.  You have not yet performed a mediation analysis in `lavaan` or `OpenMx` and you have a fitted model object.
    ...
    The first argument for these functions are the fitted model object-- you do not need to specify the coefficient estimates and their standard errors (covariance matrix of the coefficients) as the functions will extract these from the fitted model object.
